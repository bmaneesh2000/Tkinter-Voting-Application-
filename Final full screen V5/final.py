...
def initialize_app():
    ...
    sp_labels = []
    sp_names = ["Valt", "Tanjiro"]
    for i, (photo, name) in enumerate(zip([photo1, photo2], sp_names), start=1):
    ...
    asp_labels = []
    asp_names = ["ASP1", "ASP2"]
    for i, (photo, name) in enumerate(zip([photo3, photo4], asp_names), start=3):
        label_frame = tk.Frame(asp_image_frame, bg='white')  # Frame for white background
        label_frame.grid(row=0, column=i-3, padx=20, pady=20)
